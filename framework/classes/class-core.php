...
class Lapa {
    ...
    private static $original_option_name = 'lapa_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'torresdigital' );
        ...
        $this->settings       = new Lapa_Setting();
        $this->images         = new Lapa_Images();
        $this->layout         = new Lapa_Layout();
        $this->options        = new Lapa_Options();
        $this->favorite       = new Lapa_FavoritePosts();

        new Lapa_Init();
        new Lapa_Blog();
        new Lapa_Widget_CssClass();
    ...
    public function build_crumbs(){
        ...
        if( $this->settings()->get('google_rich_snippets', 'no') == 'yes' ){
            if(function_exists('wc') && is_object(wc()->structured_data)){
                add_action('lapa/action/breadcrumbs/before_render_html', array(wc()->structured_data, 'generate_breadcrumblist_data'), 10);
            }
        }
        $this->breadcrumbs = new Lapa_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['lapa_update'] ) && '1' == $_GET['lapa_update'] ) {
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Lapa_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Lapa_Multilingual::get_active_language();
        }
