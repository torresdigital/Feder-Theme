    ...
    class Lapa_WooCommerce{
        ...
        public function add_quick_view_btn(){
            if( 'on' == Lapa()->settings()->get('woocommerce_show_quickview_btn', 'off') ){
                ...
                printf(
                    ...
                    esc_attr_x('Quick Shop', 'front-view', 'torresdigital'),
                    esc_attr_x('Quick Shop', 'front-view', 'torresdigital')
                );
            }
        }
            ...
            if( Lapa()->settings()->get('woocommerce_show_compare_btn', 'off') == 'on' ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Add to Compare','front-view', 'torresdigital'),
                    ...
                    $url = '#';
                    $text = esc_html_x('Add to Compare','front-view', 'torresdigital');
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s">%s</a>',
                        esc_attr($css_class),
                        esc_url($url),
            ...
            if(Lapa()->settings()->get('woocommerce_show_wishlist_btn', 'off') == 'on'){
                ...
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'torresdigital');
                        $class = 'add_wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'torresdigital');
        ...
        public function add_custom_tab($tabs){
            if ( Lapa()->settings()->get('woo_enable_custom_tab', 'off') == 'on' ) {
                $tabs['custom_tab'] = array(
                    'title' => Lapa()->settings()->get('woo_custom_tab_title', esc_html_x('Custom Tab', 'front-view', 'torresdigital')),
                    'priority' => 40,
        ...
        public function modify_availability_text_on_product_page( $availability, $product ){
            if ( ! $product->is_in_stock() ) {
                ...
                $availability = $availability = __( 'Stock: Available', 'torresdigital' );
            }
            return $availability;
        }
        ...
        public function add_sku_to_single_product(){
            ...
            if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ){
                ?>
                <div class="product_meta product_meta-top">
                    <span class="sku_wrapper"><span class="sku"><?php echo ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'torresdigital' ); ?></span></span>
                </div>
                <?php
            }
        }
        ...
        public function modify_ajax_cart_fragments( $fragments ){
            ...
            $text = '<span class="la-cart-text">'. esc_html_x('%s items','front-view', 'torresdigital') .'</span>';
        ...
        public function add_coupon_form_into_cart(){
            if ( wc_coupons_enabled() ) : ?>
                <div class="la-coupon-form">
                    <h2><?php echo esc_html_x('Coupon Code', 'front-view', 'torresdigital') ?></h2>
                    <p><?php echo esc_html_x('Enter your coupon code if you have one.','front-view', 'torresdigital')?></p>
                    <div class="la-coupon">
                        <p class="form-row form-row-wide">
                            <input type="text" class="input-text" id="coupon_code_ref" value="" placeholder="<?php echo esc_attr_x( 'Enter your coupon code..', 'front-view', 'torresdigital' ); ?>" />
                        </p>
                        <button type="button" class="button" id="coupon_btn_ref"><?php echo esc_html_x( 'Apply coupon', 'front-view', 'torresdigital' ); ?></button>
                    </div>
                </div>
            <?php endif;
        }
        ...
        public function add_more_button_to_cart_from(){
            ...
            ?>
            <input type="submit" class="button btn-clear-cart" name="clear-cart" value="<?php echo esc_attr_x('Clear Cart', 'front-view', 'torresdigital');?>">
            <a href="<?php echo esc_url(!empty($category_recent_link) ? $category_recent_link : wc_get_page_permalink('shop')) ?>" class="btn"><?php echo esc_html_x('Continue Shopping', 'front-view', 'torresdigital'); ?></a>
            <?php
        }
        ...
        public function woocommerce_before_shipping_calculator(){
            printf(
                ...
                esc_html_x('Calculate Shipping', 'front-view', 'torresdigital'),
                esc_html_x('Estimate your shipping fee *', 'front-view', 'torresdigital')
            );
        }
