...
class Lapa_Breadcrumbs {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'page_title_format' => '<header><h1 class="page-title h3">%s</h1></header>',
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'lapa/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'full', // minified , full
			'post_taxonomy'     => apply_filters(
				'lapa/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
					'product'	=> 'product_cat'
				)
		...
		$this->args = apply_filters( 'lapa/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'lapa/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
		...
		do_action('lapa/action/breadcrumbs/before_render_html', $this, $this->args);
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'lapa/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function get_title() {
		...
		if(is_singular() && !is_page()){
			$this->args['page_title_format'] = '<header><div class="page-title h3">%s</div></header>';
		}

		$title = apply_filters(
			'lapa/filter/page_title',
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html_x( 'Browse:', 'front-view', 'torresdigital' ),
			'home'                => $this->home_title(),
			'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'torresdigital' ),
			'archives'            => esc_html_x( 'Archives', 'front-view', 'torresdigital' ),
			'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'torresdigital' ),
			'paged'               => esc_html_x( 'Page %s', 'front-view', 'torresdigital' ),
			'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'torresdigital' ),
			'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'torresdigital' ),
	...
	public function home_title() {
		...
		$title            = esc_html_x( 'Home', 'front-view', 'torresdigital' );
		$use_custom_title = apply_filters( 'lapa/filter/breadcrumbs/custom_home_title', false );
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'lapa/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 */
		$this->items = apply_filters( 'lapa/filter/breadcrumbs/items', $this->items, $this->args, $this );
	...
	public function add_post_terms( $post_id ) {
		...
		if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
			add_filter('get_object_terms', 'lapa_exclude_demo_term_in_category');
			$post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
			remove_filter('get_object_terms', 'lapa_exclude_demo_term_in_category');
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'lapa/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'lapa/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
	...
	public function get_breadcrumb() {
		return apply_filters( 'lapa/filter/breadcrumbs/get_breadcrumb', $this->crumbs, $this );
	}
}